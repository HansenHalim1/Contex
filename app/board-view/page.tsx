...
function extractMondayContext(data: unknown): MondayContextSnapshot | null {
  ...
  const camelBaseUrl = account && typeof account.baseUrl === "string" ? account.baseUrl : undefined;
  const cleanedBaseUrl =
    typeof rawBaseUrl === "string" && rawBaseUrl.trim().length >= 8
      ? rawBaseUrl.trim()
      : typeof camelBaseUrl === "string" && camelBaseUrl.trim().length >= 8
        ? camelBaseUrl.trim()
        : undefined;
  const slug = account && typeof account.slug === "string" && account.slug.trim() ? account.slug.trim() : undefined;
...
export default function BoardView() {
  ...
  const openBoardInMonday = useCallback(async (mondayBoardId: string) => {
    ...
    if (typeof baseUrl === "string" && /^https:\/\/[a-z0-9.-]+\.monday\.com/i.test(baseUrl)) {
      safeUrl = baseUrl;
    } else if (slug && /^[a-z0-9-]+$/i.test(slug)) {
      safeUrl = `https://${slug}.monday.com`;
    }
    if (!safeUrl) {
